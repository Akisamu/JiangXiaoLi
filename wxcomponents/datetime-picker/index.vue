<template>
<uni-shadow-root class="datetime-picker-index"><view class="van-picker">
  <view v-if="showToolbar" class="van-picker__toolbar van-hairline--bottom">
    <view class="van-picker__cancel" @click="onCancel">{{ cancelButtonText }}</view>
    <view class="van-picker__title">{{ title }}</view>
    <view class="van-picker__confirm" @click="onConfirm">{{ confirmButtonText }}</view>
  </view>
  ...
  <picker-view :indicator-style="'height: '+(itemHeight)+'px;'" :style="'width: 100%; height: '+(itemHeight * visibleItemCount + 'px')" @change="onChange" :value="pickerValue" class="van-picker__columns">
    <picker-view-column v-for="(row,rowIndex) in (columns)" :key="row.rowIndex" class="van-picker-column">
      <view v-for="(item,index) in (row)" :key="item" :style="'line-height: '+(itemHeight)+'px;'" :class="'van-picker-column__item '+(index === pickerValue[rowIndex] ? 'van-picker-column__item--selected' : '')">{{ item }}</view>
    </picker-view-column>
  </picker-view>
