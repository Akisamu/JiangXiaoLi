<template>
<uni-shadow-root class="search-index"><view :class="'van-search custom-class '+(showAction || useActionSlot ? 'van-search--show-action' : '')" :style="'background: '+(background)">
  <van-field clearable type="search" left-icon="search" :focus="focus" :error="error" :border="false" confirm-type="search" class="van-search__field" :value="value" :disabled="disabled" :readonly="readony" :maxlength="maxlength" :input-align="inputAlign" :placeholder="placeholder" :placeholder-style="placeholderStyle" custom-style="padding: 3px 10px" @blur="onBlur" @focus="onFocus" @change="onChange" @confirm="onSearch"></van-field>
